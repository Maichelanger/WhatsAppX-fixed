...
exports.LoadUtils = () => {
    ...
    // Message types that never carry real user-facing content: WhatsApp's
    // own protocol/system bookkeeping entries (undecryptable "ciphertext"
    // placeholders, e2e session notices, group-protocol messages, HSM/debug
    // frames, etc). The WhatsApp Legacy iOS client has no renderer for these
    // (nor should it - there's nothing to show), so both the chat list's
    // last-message preview and the full message list should skip them
    // entirely rather than surfacing a confusing placeholder.
    window.WWebJS.NO_CONTENT_MESSAGE_TYPES = [
        'ciphertext',
    ...
    window.WWebJS.getMessageModel = (message) => {
        ...
        if (!msg.id._serialized) {
            // Normally WhatsApp's own message.serialize() fills this in, but
            // partially-hydrated/backfilled messages (e.g. history synced in
            // later) can come back missing it. Without a real _serialized
            // id, the client ends up sending the literal string "(null)" to
            // the media-download endpoint. Reconstruct it from the raw
            // message key's components using WhatsApp's own id format:
            // "{fromMe}_{remoteJid}_{id}" (plus "_{participant}" in groups).
            try {
                const rawId = message.id;
                const remoteSerialized =
                    typeof rawId.remote === 'object'
                        ? rawId.remote._serialized
                        : rawId.remote;
                const participantSerialized = rawId.participant
                    ? typeof rawId.participant === 'object'
                        ? rawId.participant._serialized
                        : rawId.participant
                    : null;
                const parts = [
                    rawId.fromMe ? 'true' : 'false',
                    remoteSerialized,
                    rawId.id,
                ];
                if (participantSerialized) parts.push(participantSerialized);
                if (remoteSerialized && rawId.id) {
                    msg.id = Object.assign({}, msg.id, {
                        _serialized: parts.join('_'),
                    });
                }
            } catch (e) {
                // Leave msg.id as-is if reconstruction fails — downstream
                // validation (getMessageById) will surface a clear error
                // instead of silently sending a broken id to the client.
            }
        }
    ...
    window.WWebJS.getChat = async (chatId, { getAsModel = true } = {}) => {
        ...
        try {
            ...
            debugSteps.push('createWid:ok');
        } catch (e) {
            throw new Error(`getChat(${chatId}) failed at createWid: ${e.message}`);
        }
        ...
        if (isChannel) {
            ...
            const cachedChat = window.require('WAWebCollections').Chat.get(chatWid);
            debugSteps.push(`cachedChat:${!!cachedChat}`);

            // The chat may already be loaded in the collection but keyed
            // under a different wid than the one we built (e.g. the chat
            // is internally keyed by its LID identity while we constructed
            // a @c.us wid from a raw phone number, or vice versa). Scan the
            // already-loaded models for a matching user id before assuming
            // it needs to be freshly resolved.
            let scannedChat = null;
            if (!cachedChat && chatWid.user) {
                try {
                    const allChats = window
                        .require('WAWebCollections')
                        .Chat.getModelsArray();
                    scannedChat = allChats.find(
                        (c) => c.id && c.id.user === chatWid.user,
                    );
                    debugSteps.push(`scannedChat:${!!scannedChat}`);
                } catch (eScan) {
                    debugSteps.push(`scannedChat:threw:${eScan.message}`);
                }
            }

            if (cachedChat || scannedChat) {
                chat = cachedChat || scannedChat;
            } else {
                try {
                    ...
                    chat = result?.chat;
                    debugSteps.push(`findOrCreateLatestChat:ok:${!!chat}`);
                } catch (e) {
                    debugSteps.push(`findOrCreateLatestChat:threw:${e.message}`);

                    let alt, altErrMsg;
                    try {
                        alt = window
                            .require('WAWebApiContact')
                            .getAlternateUserWid(chatWid);
                        debugSteps.push(`getAlternateUserWid:ok:${!!alt}`);
                    } catch (e2) {
                        altErrMsg = e2.message;
                        debugSteps.push(`getAlternateUserWid:threw:${e2.message}`);
                    }

                    if (alt) {
                        try {
                            chat =
                                window.require('WAWebCollections').Chat.get(alt) ||
                                (
                                    await window
                                        .require('WAWebFindChatAction')
                                        .findOrCreateLatestChat(alt)
                                )?.chat;
                            debugSteps.push(`altLookup:ok:${!!chat}`);
                        } catch (e3) {
                            debugSteps.push(`altLookup:threw:${e3.message}`);
                        }
                    }

                    if (!chat) {
                        throw new Error(
                            `getChat(${chatId}) unresolvable. Steps: ${debugSteps.join(' | ')}`,
                        );
                    }
                }
            }
    ...
    window.WWebJS.getChats = async () => {
        ...
        // Build each chat model independently so that one chat failing
        // (e.g. "no lid for user" when a contact's LID mapping hasn't been
        // resolved/cached yet) doesn't reject the whole Promise.all and take
        // down the entire chat list.
        const results = await Promise.allSettled(
            chats.map((chat) => window.WWebJS.getChatModel(chat)),
        );

        const rejected = results
            .map((r, i) => ({ r, chat: chats[i] }))
            .filter((x) => x.r.status === 'rejected');
        if (rejected.length) {
            console.error(
                `[WWebJS.getChats] ${rejected.length}/${chats.length} chats failed to build and were dropped: ` +
                JSON.stringify(rejected.map(({ r, chat }) => ({
    ...
    window.WWebJS.getChatModel = async (chat, { isChannel = false } = {}) => {
        ...
        try {
            ...
            // chat.serialize() can throw internally (e.g. "no lid for user")
            // when WhatsApp Web hasn't resolved/cached a LID mapping for this
            // chat's contact yet. Build a minimal fallback model instead of
            // failing the whole call — same rationale as the formattedTitle
            // and participant-id guards below.
            model = {
                id: chat.id ? { _serialized: chat.id._serialized } : undefined,
                name: chat.name,
        ...
        if (isChannel) {
            ...
            try {
                ...
                // chat.formattedTitle can throw internally (e.g. "no lid for
                // user") when WhatsApp Web hasn't resolved/cached a LID
                // mapping for this contact yet. Fall back to whatever name
                // info we already have rather than failing the whole chat.
        ...
        if (chat.groupMetadata) {
            ...
            for (const p of serializedMetadata.participants || []) {
                try {
                    ...
                    // toPn can throw ("no lid for user") for a participant
                    // whose LID hasn't been resolved/cached yet. Keep the
                    // original id rather than failing the whole group chat.
                }
            }
        ...
        if (model.msgs && model.msgs.length) {
            try {
                // WhatsApp maintains chat.lastReceivedKey as a pointer to the
                // true last message, independent of whether this chat has
                // been opened in the browser session yet — so try that
                // first. Only fall back to walking the chat's loaded
                // messages (chat.msgs) if that pointer is missing/broken, or
                // resolves to a no-content system message.
                let lastMessage = null;
                const lastKeyId = chat.lastReceivedKey?._serialized;
                if (lastKeyId) {
                    const resolved =
                    ...
                    if (
                        resolved &&
                        !window.WWebJS.NO_CONTENT_MESSAGE_TYPES.includes(
                            resolved.type,
                        )
                    ) {
                ...
                // Fall back to walking this chat's already-loaded messages
                // backwards for the most recent one with real content.
                if (!lastMessage) {
                    const loadedMsgs = chat.msgs.getModelsArray();
                    for (let i = loadedMsgs.length - 1; i >= 0; i--) {
                        const candidate = loadedMsgs[i];
                        if (
                            candidate &&
                            !window.WWebJS.NO_CONTENT_MESSAGE_TYPES.includes(
                                candidate.type,
                ...
                // Don't fail the whole chat over a missing last-message
                // preview.
            }
        }
    ...
    window.WWebJS.getContactModel = (contact) => {
        ...
        if (wid.isLid() && contact.phoneNumber) {
            ...
            // Also normalize userid so that contact.number in the app
            // (which maps to data.userid via Contact.js) always matches
            // participant.id.user (which is also the real phone number
            // after toPn translation). Without this, a LID contact's
            // number field stays as the LID number and never matches.
            res.userid = contact.phoneNumber.user;
        }
    ...
    window.WWebJS.getProfilePicUrl = async (chatId) => {
        const debugSteps = [];

        const tryFind = async (wid, label) => {
            try {
                const cached = window.require('WAWebCollections').ProfilePicThumb.get(wid);
                if (cached && (cached.img || cached.eurl)) {
                    debugSteps.push(`${label}:cached:${JSON.stringify({ img: cached.img, eurl: cached.eurl })}`);
                    return cached.img || cached.eurl;
                }
                debugSteps.push(`${label}:cached:${cached ? 'stub-no-url' : 'none'}`);

                const found = await window.require('WAWebCollections').ProfilePicThumb.find(wid);
                debugSteps.push(`${label}:found:${JSON.stringify({ img: found?.img, eurl: found?.eurl })}`);
                return found?.img || found?.eurl || undefined;
            } catch (err) {
                debugSteps.push(`${label}:threw:${err.message || err.name || String(err)}`);
                return undefined;
            }
        };
        ...
        try {
            ...
            throw new Error(`getProfilePicUrl(${chatId}) failed at createWid: ${e.message}`);
        }

        let url = await tryFind(chatWid, 'primary');
        if (url) return url;
        ...
        try {
            ...
            if (chatWid.user) {
                try {
                    const allContacts = window
                        .require('WAWebCollections')
                        .Contact.getModelsArray();
                    const matched = allContacts.find(
                        (c) => c.id && c.id.user === chatWid.user && c.id._serialized !== chatWid._serialized,
                    );
                    if (matched) {
                        alt = matched.id;
                        debugSteps.push(`altScan:found:${matched.id._serialized}`);
                    } else {
                        debugSteps.push('altScan:none');
                    }
                } catch (eScan) {
                    debugSteps.push(`altScan:threw:${eScan.message}`);
                }
            }

            if (!alt) {
                alt = window.require('WAWebApiContact').getAlternateUserWid(chatWid);
                if (alt && typeof alt === 'string') {
                    alt = window.require('WAWebWidFactory').createWid(alt);
                }
            }

            if (alt) {
                url = await tryFind(alt, 'alt');
                if (url) return url;
            } else {
                debugSteps.push('alt:none');
            }
        } catch (e) {
            debugSteps.push(`alt:threw:${e.message}`);
        }

        throw new Error(`getProfilePicUrl(${chatId}) unresolvable. Steps: ${debugSteps.join(' | ')}`);
    };
    ...
    window.WWebJS.getMessageById = async (msgId) => {
        ...
        if (!msg) {
            const params = msgId.split('_');
            if (params.length !== 3) {
                throw new Error('Invalid serialized message id specified');
            }

            const messagesObject = await window
                .require('WAWebCollections')
                .Msg.getMessagesById([msgId]);

            if (messagesObject && messagesObject.messages && messagesObject.messages.length) {
                msg = messagesObject.messages[0];
            } else {
                // Fall back to searching within the chat itself, loading
                // earlier history if needed. Uses our LID-safe getChat so
                // this doesn't break for chats with unresolved LIDs.
                const chat = await window.WWebJS.getChat(params[1], { getAsModel: false });
                if (chat) {
                    const msgFilter = (m) => m.id.id === params[2];
                    let foundMessage = chat.msgs.getModelsArray().find(msgFilter);

                    let attempts = 0;
                    while (!foundMessage && attempts < 10) {
                        try {
                            const loadedMessages = await window
                                .require('WAWebChatLoadMessages')
                                .loadEarlierMsgs({ chat });
                            if (!loadedMessages || !loadedMessages.length) break;
                            foundMessage = loadedMessages.find(msgFilter);
                        } catch (e) {
                            break;
                        }
                        attempts++;
                    ...
                    if (foundMessage) msg = foundMessage;
                }
            }
        }
