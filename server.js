...
app.all("/getChats", async (req, res) => {
    ...
    if (inFlightGetChats) {
        console.log(`[getChats] Another request is already in flight — awaiting its result instead of duplicating work.`);
        try {
            const result = await inFlightGetChats;
            return res.json(result);
        } catch (error) {
            console.error(`[getChats] FAILED (from shared in-flight request): ${error.message}`);
            return res.status(500).send("Failed to get chats: " + error.message);
        }
    }
    ...
    inFlightGetChats = (async () => {
        const allChats = await Promise.race([client.getChats(), timeout(90000, 'client.getChats()')]);
        console.log(`[getChats] client.getChats() resolved with ${allChats.length} chats`);

        const chatsWithMessages = allChats.filter(chat => chat.timestamp || chat.lastMessage);
        console.log(`[getChats] ${chatsWithMessages.length} chats have timestamp/lastMessage`);
...
app.all("/getMediaData/:mediaId", async (req, res) => {
  ...
  try {
    ...
    if (isVideo) {
      ...
      const cmd = `"${ffmpegPath}" -y -i "${rawFile}" -vf "scale='min(640,iw)':'min(480,ih)':force_original_aspect_ratio=decrease:force_divisible_by=2,fps=30,yadif" -c:v libx264 -preset veryfast -crf 23 -c:a aac -b:a 160k -ar 48000 -ac 2 -movflags +faststart "${movFile}"`;
...
app.post("/sendMessage/:contactId", async (req, res) => {
    try {
        ...
        if (req.body.sendAsVoiceNote) {
            ...
            const convertedAudioPath = path.join(os.tmpdir(), `test_out_${Date.now()}.ogg`);
